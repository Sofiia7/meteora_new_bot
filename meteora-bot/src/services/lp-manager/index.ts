...
export class LpManager {
  ...
  async openPosition(
    ...
    try {
      ...
      // ВЕСЬ диапазон лежит по ОДНУ сторону от активного бина — но сторону
      // диктует не рост цены, а сам SDK (@meteora-ag/dlmm, toAmountBothSide):
      // диапазон целиком ВЫШЕ active → SDK использует только totalXAmount
      // (ask side), диапазон целиком НИЖЕ active → только totalYAmount (bid
      // side) — вне зависимости от strategyType. Поэтому:
      //   - SOL = Y → диапазон должен быть НИЖЕ active (bid side, тратим Y=SOL).
      //   - SOL = X → диапазон должен быть ВЫШЕ active (ask side, тратим X=SOL).
      // (см. computeSingleSidedBinRange в dlmm-range.ts). Раньше направление
      // было перепутано (взято из интуиции «выше = цена растёт»), из-за чего
      // SDK получал наш SOL по стороне, для которой требовался ДРУГОЙ токен, и
      // молча депонировал 0 — это и есть настоящая причина «position open, но
      // Your Liquidity = $0», а не Spot vs BidAsk.
      // priceRangeUpper по ТЗ — глубина диапазона от текущей цены в ту сторону,
      // которую выбрал SOL-side; priceRangeLower для single-sided не используется
      // (защита −90% обеспечивается тем, что наш SOL конвертируется в токен
      // постепенно, по бинам, а не сразу).
      ...
      // BidAsk концентрирует ликвидность на дальнем краю диапазона (глубже от
      // текущей цены) — подходит нашему сценарию «лимитник на весь депозит с
      // защитой от дампа» лучше, чем равномерный Spot. Строгой необходимости в
      // этом нет (по ТЗ была Spot, и с исправленным направлением диапазона она
      // тоже депонирует ненулевую сумму — see toAmountBothSide в SDK): это
      // выбор формы распределения внутри уже правильной стороны, а не фикс.
